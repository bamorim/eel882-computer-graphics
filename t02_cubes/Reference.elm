...
face : Color -> Vec3 -> Vec3 -> Vec3 -> Vec3 -> Drawable Vertex
...
renderOne : Float -> (Drawable Vertex) -> Renderable
renderOne angle drawable = render vertexShader fragmentShader drawable (uniforms angle)

scene : Float -> List Renderable
scene angle = List.map (renderOne angle) cube
